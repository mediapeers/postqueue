module Postqueue
  class Queue
    ...
    def upcoming(relation = nil) #:nodoc:
    ...
    def select_and_lock_batch(channel:, max_batch_size:)
      relation = item_class.all
      relation = relation.where(channel: channel) if channel
    ...
    def calculate_batch_size(op:, max_batch_size:)
      op = Postqueue.callback(op: op)
      return 1 unless op

      [ op.batch_size, max_batch_size ].min
    end
  end
end
